...
import (
	...
	"github.com/rickonono3/m2obj/m2yaml"
	"github.com/stretchr/testify/assert"
)

func TestFileSyncer_m2yaml_Save(t *testing.T) {
	initTestData("yml")
	formatter := m2yaml.Formatter{}
...
func TestFileSyncer_m2yaml_Load(t *testing.T) {
	initTestData("yml")
	t.Run("TestFileSyncer_m2yaml_Save", TestFileSyncer_m2yaml_Save)
	formatter := m2yaml.Formatter{}
	fs := m2obj.NewFileSyncer(filePath, formatter)
...
func TestFileSyncer_m2yaml_AutoSave(t *testing.T) {
...
func TestFileSyncer_m2yaml_AutoLoad(t *testing.T) {
	waitStartTime := 2 * time.Second
	waitStopTime := 1 * time.Second
	initTestData("yml")
	formatter := m2yaml.Formatter{}
...
func TestFileSyncer_m2yaml_BindObject(t *testing.T) {
	initTestData("yml")
	formatter := m2yaml.Formatter{}
	fs := m2obj.NewFileSyncer(filePath, formatter)
...
func TestFileSyncer_m2yaml_GetSet(t *testing.T) {
	initTestData("yml")
	fs := m2obj.NewFileSyncer(filePath, m2yaml.Formatter{})
	fs.BindObject(obj)
	assert.NotPanics(t, func() {
		assert.Equal(t, filePath, fs.GetFilePath())
		fs.SetFilePath(filePath + ".bak")
		assert.Equal(t, filePath+".bak", fs.GetFilePath())
		fs.SetFormatter(m2yaml.Formatter{})
	})
}

func TestFileSyncer_m2yaml_AutoLoadOnChange(t *testing.T) {
	initTestData("yml")
	formatter := m2yaml.Formatter{}
	fs := m2obj.NewFileSyncer(filePath, formatter)
